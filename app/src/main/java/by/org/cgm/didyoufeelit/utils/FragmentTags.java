package by.org.cgm.didyoufeelit.utils;

import by.org.cgm.didyoufeelit.fragments.DateFragment;
import by.org.cgm.didyoufeelit.fragments.EventListFragment;
import by.org.cgm.didyoufeelit.fragments.RegFormFragment;
import by.org.cgm.didyoufeelit.fragments.RegistrationFragment;
import by.org.cgm.didyoufeelit.fragments.TimeFragment;

/**
 * Author: Anatol Salanevich
 * Date: 08.05.2015
 */
public class FragmentTags {

    public static final String REGISTRATION = RegistrationFragment.class.getSimpleName();
    public static final String REG_FORM = RegFormFragment.class.getSimpleName();
    public static final String DATE = DateFragment.class.getSimpleName();
    public static final String TIME = TimeFragment.class.getSimpleName();
    public static final String EVENT_LIST = EventListFragment.class.getSimpleName();

}
